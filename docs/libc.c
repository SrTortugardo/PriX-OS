...
- putchar_col(char c, u8 color); =  writes a character on the screen using a color. char c is your character and u8 is color, for example a color would be 0x04 or 0x02
...
- scroll(); = Moves the content one line upper, to let last linea clear
- check_scroll(); = Checks if cursor is out of SCREEN_SIZE, if so, it calls scroll();

## MACROS
SCREEN_WIDTH 80 = its obvius
SCREEN_HEIGHT 25 = its obvius too
